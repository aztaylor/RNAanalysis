...
l2f_matrix <- l2f_matrix[sig_ress4, ]
...
design(se5) <- ~ genotype + treatment + genotype:treatmentdds5
design(se18) <- ~ genotype + treatment + genotype:treatment
...
contrast = list(c("treatment_ind_vs_Nind"),
                c("genotypes4.timpoint5")
)
...
sig_res5s4 <- subset(res5s4, pvalue < pval_cutoff)
sig_res18s4 <- subset(res18s4, pvalue < pval_cutoff)

sorted_sig_res5s4 <- na.omit(res5s4[order(sig_res5s4$log2FoldChange, decreasing = TRUE), ])
sorted_sig_res5s4 <- na.omit(res18s4[order(sig_res18s4$log2FoldChange, decreasing = TRUE), ])
print(sorted_sig_res5s4)

topgenes5s4 <- rownames(head(order(sig_res5s4$log2FoldChange, decreasing = TRUE), n_topgenes))
topgenes18s4 <- rownames(head(order(sig_res18s4$log2FoldChange, decreasing = TRUE),, n_topgenes))
...
l2f_filtered_df <- l2f_raw_df[!(l2f_raw_df$Targeting5hr == 0 & l2f_raw_df$Targeting18hr == 0), ]


l2f_df_tv <- l2f_filtered_df[topVarGenes, ] # Total Variance accross samples.
l2f_df_5_s4 <- l2f_filtered_df[order(abs(l2f_filtered_df$Targeting5hr), decreasing = TRUE), , drop=FALSE]

# Result
print(l2f_filtered_df)
...
gene_overlap <- rownames(nor_df) %in% rownames(l2f_filtered_df)
subset_l2f_df <- l2f_filtered_df[rownames(l2f_filtered_df) %in% rownames(nor_df), , drop=FALSE]
...
```{r}
n_topgenes <- 50
abs_sum_df <- data.frame(sort(rowSums(abs(l2f_filtered_df[1:n_topgenes, ])), decreasing=FALSE))
...
```{r}
# Fixed version of your var_l2f_df heatmap
var <- apply(l2f_df, MARGIN=1, FUN= var)
var_df = data.frame(Var=var)
rownames(var_df) <- rownames(l2f_df)

# Order by variance and keep original row order
var_l2f_df <- l2f_df[order(var_df$Var, decreasing=FALSE), , drop=FALSE]
var_l2f_df <- var_l2f_df[var_df<scale*median(var_df$Var),]

# Convert to long format while preserving order
var_l2f_df$gene <- factor(rownames(var_l2f_df), levels = rownames(var_l2f_df))
var_l2f_long <- var_l2f_df %>% 
  rownames_to_column("gene") %>%
  gather(colname, value, -gene) %>%
  mutate(
    timepoint = case_when(
      colname == "Targeting5hr" ~ "Timepoint: 5hr",
      colname == "Targeting18hr" ~ "Timepoint: 18hr"
    ),
    gene = factor(gene, levels = rownames(var_l2f_df)),  # Preserve order
    colname = factor(colname, levels=c("Targeting5hr", "Targeting18hr")),
    timepoint = factor(timepoint, levels=unique(timepoint))
  )

# Now plot
ggplot(var_l2f_long, aes(x=colname, y=gene, fill=value)) + 
  geom_tile() +
  scale_fill_distiller(palette="RdGy") +
  theme(axis.text.x = element_text(family=font, size=fsize)) +
  theme(axis.text.y = element_text(family=font, size=fsize)) +
  coord_cartesian(clip = "off") +
  facet_wrap(~timepoint, scales="free_x") +
  scale_x_discrete(labels = c("NT vs. T\n5hr.", "NT vs. T\n18hr")) +
  xlab("Treatment") +
  ylab("Gene") +
  labs(fill=expression(atop(Log[2]~"(Fold", "Change)")))
```
...
p_5_s4 <- as.data.frame(res5s4$padj)
rownames(p_5_s4) <- rownames(res5s4)
p_5_s4 <- p_5_s4[topgenes5s4, , drop=FALSE]
p_5_s4$rownames <- topgenes5s4
#colnames("WT 5hr untreated vs. st 18hr treated")

p_18_s4 <- as.data.frame(res18s4$padj)
rownames(p_18_s4) <- rownames(res18s4)
p_18_s4 <- p_18_s4[topgenes18s4, , drop=FALSE]
p_18_s4$rownames <- topgenes18s4
#colnames("WT 5hr untreated vs. st 18hr treated")

l <- list(p_5_s4, p_18_s4)
p_df <- Reduce(function(x, y) merge(x, y, by="rownames", all=T), l)
rownames(p_df) <- p_df$rownames
p_df <- p_df[,-1]
p_raw_df <- data.frame(p_df)
p_raw_df[is.na(p_raw_df)]<-0
p_df <- na.omit(p_df)
colnames(p_raw_df) <- c("Targeting5hr", "Targeting18hr")
colnames(p_df) <- c("Targeting5hr", "Targeting18hr")
p_df_tv <-p_df[topVarGenes, ] # Total Variance accross samples.
p_df_5_s4 <- p_df[order(abs(p_df$Targeting5hr), decreasing = TRUE), , drop=FALSE]
...
```{r, fig.height=12, fig.width=6}
scale = 0.7
var <- apply(p_df, MARGIN=1, FUN= var)
var_df = data.frame(Var=var)
rownames(var_df) <- rownames(p_df)

var_p_df <- p_df[order(var_df$Var, decreasing=FALSE), , drop=FALSE]
var_p_df <- var_p_df[var_df<scale*median(var_df$Var),]
var_p_df


var_p_df <- var_p_df %>% rownames_to_column() %>% gather(colname, value, -rowname)

var_p_df <- var_p_df %>% mutate(timepoint = case_when(
  colname == "Targeting5hr" ~ "Timepoint: 5hr",
  colname == "Targeting18hr" ~ "Timepoint: 18hr"
))


var_p_df$colname  <- factor(var_p_df$colname, levels=c("Targeting5hr", "Targeting18hr"))
var_p_df$timepoint <- factor(var_p_df$timepoint, levels=unique(var_p_df$timepoint))
var_p_df
var_p_df$rowname<-factor(var_p_df$rowname, levels=unique(var_p_df$rowname))

columns <- c("NT vs. T\n5hr.", "NT vs. T\n18hr")

fsize = 10

top_hits <- ggplot(var_p_df, aes(x=colname, y=rowname, fill=value)) + 
  geom_tile() +
  scale_fill_distiller(palette="RdGy") +
  theme(axis.text.x = element_text(family=font, size=fsize)) +
  theme(axis.text.y = element_text(family=font, size=fsize)) +

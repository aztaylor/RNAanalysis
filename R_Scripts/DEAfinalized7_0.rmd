...
# 3. Functional Groups (example)
set.seed(123)
gene_groups <- data.frame(
  Gene = unique(heatmap_data$Gene),
  Function = sample(c("DNA Repair","Metabolism","Signaling"), 
                   length(unique(heatmap_data$Gene)), replace = TRUE)
)
...
plots[[4]] <- heatmap_data %>%
  ...
  mutate(Gene = factor(Gene, levels = gene_groups %>%
                        arrange(Function, desc(abs(log2FoldChange))) %>%
                        pull(Gene))) %>%
  ggplot(aes(x = GenoType, y = Gene, fill = log2FoldChange)) +
  geom_tile() +
  facet_grid(Function ~ ., scales = "free_y", space = "free") +
  ggtitle("Grouped by Function")

# Method 4: Up/Down Regulation
plots[[5]] <- heatmap_data %>%
  mutate(Gene = factor(Gene, levels = direction_order)) %>%
  ggplot(aes(x = GenoType, y = Gene, fill = log2FoldChange)) +
  geom_tile() +
  ggtitle("Separated by Regulation Direction")+
  labs(
    x = "Genotype",
    y = "Gene",
    fill = "Log2FC\n(Ind vs Nind)",

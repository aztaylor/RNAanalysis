...
divergence_data <- heatmap_data %>%
  ...
  mutate(
    s4_vs_WT = abs(`s4` - `WT`),  # Absolute difference from WT baseline
    s4_vs_NC = abs(`NC` - `WT`),  # Absolute difference from NC control
    combined_divergence = s4_vs_WT + s4_vs_NC  # Total divergence score
  ) %>%
  arrange(combined_divergence) %>% # Sort by highest divergence
  rev()
...
wt_baseline <- heatmap_data_sorted %>%
  filter(genotype == "WT",
        Gene %in% gene_order) %>%
  select(Gene, treatment, timepoint, wt_l2fc = log2FoldChange)
...
ggplot(heatmap_data_sorted, aes(
    ...
    y = factor(Gene, levels = top_n_genes))
) +
  scale_y_discrete(limits = top_n_genes) +
...
relative_data <- heatmap_relative %>%
  filter(genotype %in% c("NC", "s4")) %>%
  inner_join(wt_baseline, by = c("Gene", "treatment", "timepoint")) %>%
  mutate(
    relative_l2fc = log2FoldChange - wt_l2fc,
    genotype = paste0(genotype, "-WT")
  ) %>%
  select(-log2FoldChange, -wt_l2fc) %>%
  rename(log2FoldChange = relative_l2fc)

relative_heatmap <- ggplot(relative_data, 
...
heatmap_data_sorted %>%
  filter(genotype %in% c("NC", "s4")) %>%
  anti_join(wt_baseline, by = c("Gene", "treatment", "timepoint")) %>%
  count(Gene)  # Genes missing WT baselines

wt_baseline %>% 
  count(Gene, treatment, timepoint) %>% 
  filter(n > 1)  # Check for duplicates
```

```{r}
# 1. Check intermediate results BEFORE binding
nc_results <- results(dds, list(c("genotype_NC_vs_WT", "treatment_ind_vs_Nind",
    "genotypeNC.treatmentind", "timepoint_18_vs_5",
    "genotypeNC.timepoint18", "treatmentind.timepoint18",
    "genotypeNC.treatmentind.timepoint18"))) %>% 
  as.data.frame() %>% 
  rownames_to_column("Gene") %>%
  mutate(genotype = "NC", treatment = "ind", timepoint = "18")

s4_results <- results(dds, contrast = list(c("genotype_s4_vs_WT", "treatment_ind_vs_Nind", "genotypes4.treatmentind", "timepoint_18_vs_5",
      "genotypes4.timepoint18", "treatmentind.timepoint18",
      "genotypes4.treatmentind.timepoint18"
      ))) %>% 
  as.data.frame() %>% 
  rownames_to_column("Gene") %>%
  mutate(genotype = "s4", treatment = "ind", timepoint = "18")

# Verify each component
table(nc_results$treatment)  # Should show all "ind"
table(s4_results$treatment)  # Should show all "ind"

# 2. Check AFTER binding but BEFORE any other operations
temp_combined <- bind_rows(nc_results, s4_results)
table(temp_combined$genotype, temp_combined$treatment)

# 3. Check FINAL output
table(heatmap_data$genotype, heatmap_data$treatment)
```
```{r}
...
print(l2f_heatmap)
# Save the plot
ggsave("../Figures/enhanced_heatmap_v8.png", 
       l2f_heatmap, 
       dpi = 600, 
       height = fig_height, 
       width = fig_width)
...
timepoints <- paste("timepointpoint: ", timepoint, "hr.")
...
varCounts_l2f_df <- varCounts_l2f_df %>%
  mutate(
    timepoint = paste("timepointpoint: ", as.numeric(str_extract(colname, "(?<=_)[0-9]+")), "hr.")  # Extract the number after the underscore and convert to numeric
  )
...
as_l2f_df <- as_l2f_df %>% mutate(timepoint = case_when(
  colname == "Targeting5hr" ~ "timepointpoint: 5hr",
  colname == "Targeting18hr" ~ "timepointpoint: 18hr"
))

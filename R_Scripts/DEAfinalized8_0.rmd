...
function_df <- all_genes %>%
  ...
  mutate(
    function_term = str_remove(pairs, "\\s*\\[[^]]+\\]\\s*") %>% str_trim(),
    go_term = str_extract(pairs, "\\[[^]]+\\]")
  ) %>%
  # Remove unwanted IDs
  filter(!str_detect(function_term, "^[A-Z0-9]{6,}$")) %>%
  # Remove duplicates (same gene + same term pair)
  distinct(Gene, function_term, go_term, .keep_all = TRUE)
...
function_descriptions <- function_df$function_term
...
BoW_dtm <- function_df %>%
  unnest_tokens(word, function_term) %>%
  count(Gene, word) %>%
...
BoW_lda_dtm <- removeSparseTerms(BoW_dtm, sparse = 0.95)  # Start with 0.95, adjust as needed
...
it <- itoken(function_df$function_term, 
...
library("ggpubr")
library("factoextra")
kmeans = function_df$BoW_k_means_cluster
fviz_cluster(t2v_kmeans_res, 
             data = BoW_dtm)
  theme_minimal() +
...
print(cluster_comp)
```
# Extract the LDA topics from the topic model.
```{r}
library(topicmodels)
library(tidytext)

# Assuming `lda_model` is your fitted LDA model
top_terms <- tidy(BoW_lda_model, matrix = "beta") %>%  # Term-topic probabilities
  group_by(topic) %>%
  slice_max(beta, n = 10) %>%
  ungroup() %>%

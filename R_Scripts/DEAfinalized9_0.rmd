...
combined_network <- analyzer$combine_networks(gamma_threshold = 0.15,
                                              dd_gamma_threshold = 0.99)
...
ggraph(combined_network, layout = "kk") +
  ...
  geom_node_point(
    aes(color = `group`, size = ifelse(type == "document", 2, alpha))
  ) +
...
```{r}
combined_es <- analyzer$combined_edges
```

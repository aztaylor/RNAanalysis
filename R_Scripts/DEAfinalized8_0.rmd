...
```{r}
genes <- heatmap_data_sorted$Gene

# Fetch UniProt data for all genes
results <- lapply(genes, function(gene) {
  url <- paste0("https://www.ebi.ac.uk/proteins/api/proteins?gene=", gene)
  response <- GET(url, accept("application/json"))
  fromJSON(rawToChar(response$content))
})

# Extract functions
sapply(results, function(x) x$protein$recommendedName$fullName$value)
```

```{r}
functions <- sapply(results, function(x) x$protein$recommendedName$fullName$value)

# Function to safely extract GO terms
...
all_genes <- mapIds(
  ...
  keytype = "SYMBOL",
  column = "ENTREZID",
  multiVals = "first"  # Returns all matches 
) %>% unique()

print(all_genes)

go_results <- lapply(all_genes, function(gene) {
  res <- select(up, keys = gene, columns = c("cc_function"), keytype = "GeneID")
  if (length(rownames(res)) > 0) {
    func <- grepl(res$Function..CC., pattern = "FUNCTION: (.*?) \\{", ignore.case = TRUE)
    sources <- grepl(res$Function..CC., pattern = "(.\\{*?\\})", ignore.case = TRUE)
    data.frame(Gene = gene, Function = func, Sources = sources)#, Description = terms[2])
  } else {
    data.frame(Gene = gene, Function = "No function description.", Sources = NA)
  }
})
...
# Create a mapping between symbols and ENTREZ IDs
...
function_results <- lapply(1:nrow(gene_mapping), function(i) {
  gene_symbol <- gene_mapping$SYMBOL[i]
  gene_entrez <- gene_mapping$ENTREZID[i]
  
  res <- select(up, keys = gene_entrez, columns = c("cc_function"), keytype = "GeneID")
  
  if (length(rownames(res)) > 0) {
    # Extract function and sources
    func_match <- str_match(res$Function..CC., "FUNCTION: (.*?) \\{")
    sources_match <- str_match(res$Function..CC., "\\{(.*?)\\}")
    
    data.frame(
      Gene_Symbol = gene_symbol,  # Original symbol
      Gene_ENTREZ = gene_entrez,  # ENTREZ ID
      Function = ifelse(!is.na(func_match[,2]), func_match[,2], NA),
      Sources = ifelse(!is.na(sources_match[,2]), sources_match[,2], NA),
      stringsAsFactors = FALSE
    )
  } else {
    data.frame(
...
# Combine results
function_df <- do.call(rbind, function_results)
print(function_df)
```

...
txi <- tximport(files = paste(quant_fp, quant_files, sep = '/'),
                      type = "salmon",
                      txOut = TRUE)
```
```{r}
print(paste(quant_fp, quant_files, sep = '/'))
```
...
conditionNC1NCind1 <- ifelse(grepl("NC_1", sample_names), "NC", 
                     ifelse(grepl("NC_1_ind", sample_names), "NC_ind_1", NA))
sampleTableNC1NCind1 <- data.frame(sampleName = sample_names,
                          condition = factor(conditionNC1NCind1))
...
print(conditionNC1NCind1)
```

```{r}
filtered_sampleTableNC1NCind1 <- sampleTableNC1NCind1[!is.na(sampleTableNC1NCind1$conditionNC1NCind1), ]
fileterd_txiNC1NCind1 <- txi$counts[, filtered_sampleTableNC1NCind1$sampleName]
```

```{r}
ddsNC1NCind1 <- DESeqDataSetFromTximport(
  txi = fileterd_txiNC1NCind1,
  colData = filtered_sampleTableNC1NCind1,
  design = ~ conditionNC1NCind1
)
```

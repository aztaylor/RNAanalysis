...
# Plasmid effect at T1 (NC -IPTG vs. WT -IPTG)
res_NC_vs_WT_T1 <- results(dds, contrast = c("genotype", "NC", "WT"))

# Induction effect at T2 (s4 +IPTG vs. NC +IPTG)
res_s4_vs_NC_T2 <- results(dds, list(
  c("genotype_s4_vs_WT", "treatment_ind_vs_Nind", "genotypes4.treatmentind"),
  c("genotype_NC_vs_WT", "treatment_ind_vs_Nind", "genotypeNC.treatmentind")
))

# Time effect in s4 +IPTG (T2 vs. T1)
res_s4_T2_vs_T1 <- results(dds, contrast = list(
  "timpoint_T2_vs_T1", "genotpes4.timpoint18", "treatmentind.timpoint18", "genotypes4.treatmentind.timpoint18"
))
...
sig_genes <- unique(c(
  rownames(subset(res_NC_vs_WT_T1, padj < 0.05 & abs(log2FoldChange) > 1 & !grepl(rRNAgenes, rownames(res_NC_vs_WT_T1)))),
  rownames(subset(res_s4_vs_NC_T2, padj < 0.05 & abs(log2FoldChange) > 1 & !grepl(rRNAgenes, rownames(res_s4_vs_NC_T2)))),
  rownames(subset(res_s4_ind_T2_vs_t2, padj < 0.05 & abs(log2FoldChange) > 1 & !grepl(rRNAgenes, rownames(res_s4_ind_T2_vs_t2))))
))

# Now create data frames from each of the results and
# and assign genes and genotypes data.
WT_5hr_df <- data.frame(
  Gene = rownames(res_s4_T2_vs_t2),
  log2FoldChange = res_s4_T2_vs_t2$log2FoldChange,
  padj = res_s4_ind_T2_vs_t2$padj,
  GenoType = "WT"
)
NC_5hr_df <- data.frame(
  Gene = rownames(res_NC_vs_WT_T1),
  log2FoldChange = res_NC_vs_WT_T1$log2FoldChange,
  padj = res_NC_vs_WT_T1$padj,
  GenoType = "NC"
)
s4_5hr_df <- data.frame(
  Gene = rownames(res_s4_vs_NC_T2),
  log2FoldChange = res_s4_vs_NC_T2$log2FoldChange,
  padj = res_s4_vs_NC_T2_ind$padj,
  GenoType = "s4"
)
